...
namespace lib102
{
    ...
    /// <para>传送原因：下行（请求5），上行（13：无，14无）</para>
    /// <para>  控制方向：请求（5）</para>
    ...
    /// <summary>
    /// 读终端当前时间 C_TI_NA_2(100)
    /// <para>传送原因：下行（请求5），上行（14无）</para>
    /// <para>  控制方向：请求（5）</para>
    /// <para>  监视方向：无所请求的应用服务数据单元（14）</para>
    /// <para>1. 不含信息体</para>
    /// <para>2. 如果有数据，用类型标识72（当前时间）回答，7字节时标回答</para>
    /// <para>3. 如果没有相关信息，用镜像报文未出回答(14)，修改传送原因</para>
    /// </summary>
    public class ReadCurrentTime : InformationObject
    {
        /// <summary>
        /// TypeID.C_TI_NA_2 103;
        /// </summary>
        override public TypeID Type
        {
            get
            {
                return TypeID.C_TI_NA_2;
            }
        }
        ...
        /// <param name="msg">信息体内容</param>
        /// <param name="startIndex">信息体首地址</param>
        public ReadCurrentTime(byte[] msg, int startIndex)
            : base(msg, startIndex, true)
        {
            //最后一个isSequence强制标true，相当于不执行base，
            //因为base要解析信息对象地址，这里没有信息对象地址。。直接解析内容就好了
